...
CLI.prototype.putMappings = function(fileDirectory, options){
    ...
    sync.fiber(function(){
        try{
            ...
            console.log('An error has ocurred: %s'.red, JSON.toString(e));
        }
    });
};
    ...
    if(typeof fromDB != 'undefined'){
        if(typeof fromFile === 'object'){
            _.forEach(_.keys(fromFile), function(key){
                if(!upsert){
                    upsert = me.checkChilds(fromFile[key], fromDB[key]);
                }
            });
        }else{
...
CLI.prototype.deleteIndex = function(indexs, options){
    var me = this;
    ...
    sync.fiber(function(){
        try{
            ...
            console.log('The index %s was deleted correctly'.green, indexs.toString());
        }catch(e){
            console.log('An error has ocurred: %s'.red, e.message);
        }
    });
